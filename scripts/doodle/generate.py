...
from doodle import INK, PENCIL, Canvas, capsule, circles_mask, ellipse, outline_of  # noqa: E402
...
def pine(c, x, y, s, fill=PINE, ink=PENCIL, w=2.6, base=0.35, snow=True):
    ...
    if snow:
        c.line([(x - s * 0.05, y - s * 0.95), (x, y - s * 1.02), (x + s * 0.05, y - s * 0.95)], SNOW, max(2, w), amp=0.3)
...
def cabin(c, x, y, s, wall=RED, ink=PENCIL, w=2.6, base=0.38, smoke=True):
    ...
    c.shape([(x - s * 0.12, y - s * 0.66), (x + s * 0.5, y - s * 1.12), (x + s * 1.12, y - s * 0.66)], (236, 238, 244), ink, w, base=0.55, angle=0.3)
    c.shape([(x + s * 0.18, y - s * 0.5), (x + s * 0.4, y - s * 0.5), (x + s * 0.4, y - s * 0.3), (x + s * 0.18, y - s * 0.3)], (250, 214, 120), ink, w, base=0.65)
    c.shape([(x + s * 0.6, y), (x + s * 0.8, y), (x + s * 0.8, y - s * 0.42), (x + s * 0.6, y - s * 0.42)], (150, 90, 70), ink, w, base=0.45)
    ...
    if smoke:
        ...
        c.line(sm, (150, 150, 165), max(1.8, w * 0.8), amp=0.6)


def rock(c, x, y, s, ink=PENCIL, w=2.6):
    pts = [(x - s, y), (x - s * 0.8, y - s * 0.6), (x - s * 0.2, y - s * 0.9), (x + s * 0.5, y - s * 0.7), (x + s, y - s * 0.1), (x + s * 0.9, y)]
    c.shape(pts, (190, 186, 196), ink, w, base=0.35, angle=1.3)
    c.line([(x - s * 0.6, y - s * 0.55), (x - s * 0.1, y - s * 0.85), (x + s * 0.3, y - s * 0.68)], SNOW, w + 1, amp=0.3)


def mountain(c, x0, x1, base_y, peak_x, peak_y, ink=PENCIL, w=2.6, fill=(200, 204, 222)):
    ...
    c.shape(cap, SNOW, ink, w * 0.9, base=1, amp=0.8)
...
def lake_far():
    ...
    c.shape(hills, (206, 208, 228), None, 0, base=0.22, angle=1.2, gap=8, stroke=4)
    c.line(hills[:6], PENCIL, 2.4, amp=2.5)
    c.line([(-10, 848), (730, 846)], (150, 146, 166), 2, amp=1.5)
    for i, x in enumerate(range(20, 720, 46)):
        pine(c, x + (i % 3) * 7, 845 - (i % 2) * 12, 34 + (i % 3) * 6, PINE_PALE, (140, 136, 156), 1.8, 0.22, snow=False)
    return c
...
def lake_near():
    ...
    for i, y in enumerate(range(1122, 1280, 30)):
        x0 = 30 + (i * 83) % 200
        c.line([(x0, y), (x0 + 180 + (i * 37) % 120, y + 3)], (84, 124, 196), 2.2, amp=1.2)
    # Ufer mit Schnee
    ...
    c.shape(snow, SNOW, None, 0, base=0.9)
    # Steg
    ...
    cabin(c, 40, 1082, 110, wall=(170, 118, 88))
    pine(c, 650, 1085, 190)
    pine(c, 575, 1090, 120)
    pine(c, 12, 1110, 90)
    return c
...
def forest(layer):
    ...
    if layer == 'far':
        for i in range(16):
            ...
            y = r.u(120, 1240)
            s = r.u(28, 44)
            pine(c, x, y, s, PINE_PALE, (150, 146, 166), 1.7, 0.18, snow=False)
    elif layer == 'mid':
        for i in range(12):
            left = i % 2 == 0
        ...
        for k in range(5):
            c.line([(20 + k * 20, 694), (30 + k * 20, 710)], (112, 168, 116), 3, amp=0.3)
    return c
...
def fjell(layer):
    ...
    if layer == 'mid':
        ...
        for i, y in enumerate(range(200, 1280, 210)):
            ...
            if i % 3 == 1:
                for k in range(3):
                    c.line([(x + 40 + k * 8, y), (x + 44 + k * 8, y - 14)], (120, 170, 110), 2.4, amp=0.3)
    return c
...
def cloud(seed, w=562, h=180):
    ...
    c.shape(pts, SNOW, PENCIL, 7, base=1, amp=1.4)
    c.line([(w * 0.3, h - 40), (w * 0.62, h - 36)], (200, 204, 222), 4, amp=0.8)
    return c
...
def moon():
    ...
    c.shape(pts, (246, 220, 120), INK, 9, base=0.9, gap=14, stroke=16, amp=1.6)
    c.dot(130, 250, 12, (222, 186, 86))
    c.dot(110, 176, 8, (222, 186, 86))
    c.dot(176, 318, 9, (222, 186, 86))
    return c
...
def star():
    ...
    c.shape(pts, (246, 206, 70), INK, 3, base=1, stroke=4, gap=4, amp=0.6)
    return c
...
def main():
    print('Kritzel-Grafiken nach', OUT)
    ...
    save(empty(), 'bg/lake/midglow.png')
    save(empty(), 'bg/lake/nearglow.png')
