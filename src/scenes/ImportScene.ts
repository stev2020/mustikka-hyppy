...
import { COLORS, dimmer, fitText, makeButton, makeText, panel, setupCamera, type Button } from '../render/ui';
...
export class ImportScene extends Phaser.Scene {
  ...
  create(data: Data): void {
    ...
    const title = makeText(this, cx, top + 106, `„${this.deck.title}“ · ${this.deck.notes.length} Karten`, { size: 22, weight: 700, color: COLORS.muted, strokeThickness: 0 });
    fitText(title, DESIGN_W - 120, 22, 14);
    L.add(title);

    // Feldzuordnung
    const rowY = top + 180;
    L.add(makeText(this, 80, rowY - 38, 'Lernsprache (auf den Planken)', { size: 18, weight: 700, color: COLORS.muted, strokeThickness: 0 }).setOrigin(0, 0.5));
    ...
    L.add(makeText(this, 80, rowY + 62, 'Übersetzung (Frage oben)', { size: 18, weight: 700, color: COLORS.muted, strokeThickness: 0 }).setOrigin(0, 0.5));
    ...
    L.add(g);
    L.add(makeText(this, cx, pvTop + 30, 'Vorschau', { size: 20, weight: 800, color: COLORS.orange, strokeThickness: 0 }));
    for (let i = 0; i < 8; i++) {
      const t = makeText(this, cx, pvTop + 76 + i * 44, '', { size: 23, weight: 700, strokeThickness: 4 });
      this.preview.push(t);
      L.add(t);
    }
    this.statsText = makeText(this, cx, pvTop + 460, '', { size: 19, weight: 600, color: COLORS.text2, strokeThickness: 0, wrapWidth: DESIGN_W - 140 });
    L.add(this.statsText);
    this.errorText = makeText(this, cx, bottom - 180, '', { size: 20, weight: 700, color: COLORS.red, strokeThickness: 0 });
