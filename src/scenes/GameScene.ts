...
import { COLORS, fadeInCamera, fitText, makeButton, makeText, panel, setupCamera, sketchRect } from '../render/ui';
...
export class GameScene extends Phaser.Scene {
  ...
  private createHud(): void {
    ...
    this.streakText = makeText(this, DESIGN_W - 40, 92, '', { size: 18, weight: 800, color: COLORS.orange, strokeThickness: 3 }).setOrigin(1, 0.5);
    ...
    this.promptHint = makeText(this, DESIGN_W / 2, 92, `${from} → ${to}`, { size: 18, weight: 600, color: COLORS.muted, strokeThickness: 0 });
  ...
  private announce(text: string, o: { label?: string; color?: string; glow?: number; hold?: number } = {}): void {
    ...
    parts.push(glow, bg);
    if (o.label) parts.push(makeText(this, 0, -46, o.label, { size: 22, weight: 700, color: COLORS.muted, strokeThickness: 0 }));
    parts.push(word);
  ...
  private highscoreToast(): void {
    const t = makeText(this, DESIGN_W / 2, HUD_H + 40, 'Neuer Highscore!', { size: 34, weight: 900, color: COLORS.orange, strokeThickness: 7 });
  ...
  private berryArrived(): void {
    ...
    if (this.hearts < HEARTS) {
      ...
      this.floatText(DESIGN_W / 2, worldY(this.ph) - 150, '+1 Herz!', COLORS.heartPlus);
    } else {
      this.score += SPECIALS.berryBonusPoints;
      this.ctx.sfx.play('heartUp');
      this.floatText(DESIGN_W / 2, worldY(this.ph) - 150, `+${SPECIALS.berryBonusPoints} Beerenbonus!`, COLORS.blue);
    }
  }
  ...
  private answerCorrect(row: Row, p: WordPlank): void {
    ...
    if (this.streak % 5 === 0) {
      ...
      this.time.delayedCall(250, () => this.floatText(DESIGN_W / 2, worldY(p.h) - 120, `${this.streak} richtig in Folge!`, COLORS.orange));
    }
    this.score += pts;
    this.floatText(p.x, worldY(p.h) - 40, `+${pts}`, COLORS.green);
  }
  ...
  private answerWrong(row: Row, p: WordPlank): void {
    ...
    this.shownRowId = -2;
    this.setPrompt(`${q.prompt} = ${q.answer}`, COLORS.orange);
    if (this.hearts > 0) this.announce(q.answer, { label: `„${q.prompt}“ heißt`, color: COLORS.green, glow: 0x5dff84, hold: ANNOUNCE.solutionHold });
